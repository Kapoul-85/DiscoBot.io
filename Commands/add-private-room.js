...
module.exports = {
    ...
    description: "Permet d'inviter un member dans votre private room",
        ...
        try {
                ...
                if(data[i].guildID == guild.id){
                    ...
                    try {
                        const embedSucces = new Discord.EmbedBuilder()
                            .setTitle(`Invitation sur un private room !`)
                            .setColor(data[i].colorBase)
                            .setDescription(`Tu as était inviter dans un private room`)
                            .setTimestamp();
                    ...
                    const embedSucces = new Discord.EmbedBuilder()
                        .setTitle(`Succes!`)
                        .setColor(data[i].colorBase)
                        .setDescription(`${utilisateur} a bien était inviter dans votre private room`)
                        .setTimestamp();
            ...
            for (let i = 0; i < data.length; i++) {
                if(data[i].guildID == guild.id){
                    let utilisateur = args.getUser("member");
                    const embedInconnue = new Discord.EmbedBuilder()
                        .setTitle(`Pas de membre a ajouter! erreur`)
                        .setColor(data[i].colorDanger)
                        .setDescription(`Une erreur c'est porduite lors de la tantative d'ajouter ${utilisateur} dans votre private room`)
                        .setTimestamp();
